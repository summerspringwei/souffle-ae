...
docker exec -it -e SOUFFLE_RUN=${SOUFFLE_RUN} ${souffle_container_id} \ 
  /bin/bash -c "/workspace/souffle-models/python/models/bert/run_nsys_bert.sh"

# ResNext
docker exec -it -e SOUFFLE_RUN=${SOUFFLE_RUN} ${souffle_container_id} \ 
  /bin/bash -c "/workspace/souffle-models/python/models/resnext/run_nsys_resnext.sh"

# LSTM
docker exec -it -e SOUFFLE_RUN=${SOUFFLE_RUN} ${souffle_container_id} \ 
  /bin/bash -c "/workspace/souffle-models/python/models/lstm/run_nsys_lstm.sh"

# EfficientNet
docker exec -it -e SOUFFLE_RUN=${SOUFFLE_RUN} ${souffle_container_id} \ 
  /bin/bash -c "/workspace/souffle-models/python/models/efficientnet/run_nsys_efficientnet.sh"
