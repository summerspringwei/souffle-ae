...
if [ -n "${SOUFFLE_RUN}" ] && [ "${SOUFFLE_RUN}" = "TRUE" ]; then
ncu --metrics dram__bytes_read,gpu__time_duration --clock-control none -o ncu-${NAME} -f \
...
ncu -i ./ncu-${NAME}.ncu-rep --csv --page raw | grep -v "void FillKernel*" > ncu-${NAME}.csv
APOLLO_MMOE_MEM=$(python3 extract_ncu_cuda_mem_read.py ncu-${NAME}.csv)
APOLLO_MMOE_NUM_KERNELS=$(wc -l ncu-${NAME}.csv | awk '{ print $1 }')


echo "Apollo number of kernels:", ${APOLLO_BERT_NUM_KERNELS}, ${APOLLO_RESNEXT_NUM_KERNELS}, \
...
echo "Apollo memory read:", ${APOLLO_BERT_MEM}, ${APOLLO_RESNEXT_MEM}, \
